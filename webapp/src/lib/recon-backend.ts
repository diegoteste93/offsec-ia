...
function getEnvReconOrchestratorUrl(): string | undefined {
  if (typeof process === 'undefined' || !process.env) return undefined
  return process.env.RECON_ORCHESTRATOR_URL
}

function getDefaultReconBackends(): Array<string | undefined> {
  return [
    getEnvReconOrchestratorUrl(),
...
export function getReconBackendCandidates(extraCandidates: string[] = []): string[] {
  ...
  for (const candidate of [...extraCandidates, ...getDefaultReconBackends()]) {
